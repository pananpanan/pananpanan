...
data<-read.xlsx("ttest1-1.eg.xlsx ",sheet = 1,sep="\t",rowNames=TRUE)
#data<-read.xlsx("ttest1.eg.xlsx ",sheet = 1,sep="\t",rowNames=TRUE)
sampleinfos<-read.xlsx("ttest3.eg.xlsx",sheet = 1,sep="\t",rowNames=F,colNames = F)
groups<-read.xlsx("ttest2.eg.xlsx",sheet = 1,sep="\t",rowNames=F,colNames = F)

#0值填充
data[data==0]=min(data[data!=0])*0.5
#diffall=data.frame()
diffall = data.frame(matrix(
  vector(), 0, 3, dimnames=list(c(), c("group",'UP', 'DOWN'))),
  stringsAsFactors=F)
for (j in 1:length(rownames(groups)) ){
  group=as.vector(groups[j,])
  sampleindex<-lapply(group,function(x) {which(sampleinfos[,"X2"] %in% x)})
  ...
  write.table(data2, file=paste(group[1],"-vs-",group[2],".Ttest.fillter.txt",sep = ""),sep="\t",quote=FALSE,row.names = T,col.names=NA)
  write.csv(data2,file=paste(group[1],"-vs-",group[2],".Ttest.fillter.CSV",sep = ""),quote=F,row.names=T)

  diff=data.frame(paste(group[1],"-vs-",group[2],".Ttest.fillter",sep = ""),sum(data2$UPDOWN=="Up"),sum(data2$UPDOWN=="Down"))
  #rownames(diff)=paste(group[1],"-vs-",group[2],".Ttest.fillter",sep = "")
